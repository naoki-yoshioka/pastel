#ifndef PASTEL_PARTICLE_SPH_PARTICLE_HPP
# define PASTEL_PARTICLE_SPH_PARTICLE_HPP
  ...
  namespace particle
  {
    ...
    namespace sph_particle_detail
    {
      ...
      struct get< ::pastel::particle::tags::position, dimension_, Value, Point, Vector>
      {
        ...
        using particle_type = ::pastel::particle::sph_particle<dimension_, Value, Point, Vector>;
    ...
    class sph_particle final
    {
     ...
     public:
      constexpr sph_particle() noexcept = default;
      ~sph_particle() noexcept = default;
      sph_particle(sph_particle const&) = default;
      sph_particle(sph_particle&&) noexcept = default;
      sph_particle& operator=(sph_particle const&) & = default;
      sph_particle& operator=(sph_particle&&) & noexcept = default;

      constexpr sph_particle(
        point_type const& position, vector_type const& velocity, vector_type const& force,
        scalar_type const& mass, scalar_type const& density)
        : position_{position},
          velocity_{velocity},
          force_{force},
          mass_{mass},
      ...
      constexpr sph_particle(
        ...
        scalar_type&& mass, scalar_type&& density)
     ...
     private:
      ...
      friend class ::pastel::particle::sph_particle_detail::get;

      template <typename Tag>
      using sph_particle_get = ::pastel::particle::sph_particle_detail::get<Tag, dimension, Value, Point, Vector>;

     public:
      template <typename Tag>
      constexpr typename sph_particle_get<Tag>::reference get() & noexcept
      { return sph_particle_get<Tag>::call(*this); }

      template <typename Tag>
      constexpr typename sph_particle_get<Tag>::const_reference get() const& noexcept
      { return sph_particle_get<Tag>::call(*this); }

      template <typename Tag>
      constexpr typename sph_particle_get<Tag>::value_type get() && noexcept
      { return std::move(sph_particle_get<Tag>::call(*this)); }
    }; // class sph_particle<dimension_, Value, Point, Vector>

    template <std::size_t dimension_, typename Value, typename Point, typename Vector>
    inline bool operator==(
      ::pastel::particle::sph_particle<dimension_, Value, Point, Vector> const& lhs,
      ::pastel::particle::sph_particle<dimension_, Value, Point, Vector> const& rhs)
    {
      return
    ...
    template <std::size_t dimension_, typename Value, typename Point, typename Vector>
    inline void swap(
      ::pastel::particle::sph_particle<dimension_, Value, Point, Vector>& lhs,
      ::pastel::particle::sph_particle<dimension_, Value, Point, Vector>& rhs)
...
#endif // PASTEL_PARTICLE_SPH_PARTICLE_HPP
