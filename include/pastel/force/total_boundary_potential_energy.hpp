#ifndef PASTEL_FORCE_TOTAL_BOUNDARY_POTENTIAL_ENERGY_HPP
# define PASTEL_FORCE_TOTAL_BOUNDARY_POTENTIAL_ENERGY_HPP
...
namespace pastel
{
  namespace force
  {
    namespace total_boundary_potential_energy_detail
    {
      template <bool is_partner_data_accessible>
      struct total_boundary_potential_energy;

      template <>
      struct total_boundary_potential_energy<true>
      {
      ...
      template <>
      struct total_boundary_potential_energy<false>
      {
        template <typename Force, typename NeighborList, typename System>
        static typename ::pastel::force::meta::value_of<Force>::type call(
          Force const& force, NeighborList const& neighbor_list, System const& system,
    ...
    template <typename Force, typename NeighborList, typename System>
    inline typename ::pastel::force::meta::value_of<Force>::type total_boundary_potential_energy(
      Force const& force, NeighborList const& neighbor_list, System const& system)
    {
      static_assert(
      ...
      return ::pastel::force::total_boundary_potential_energy_detail::total_boundary_potential_energy<is_partner_data_accessible>::call(
...
#endif // PASTEL_FORCE_TOTAL_BOUNDARY_POTENTIAL_ENERGY_HPP
